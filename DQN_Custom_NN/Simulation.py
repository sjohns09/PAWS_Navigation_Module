...
from PAWS_Bot_Navigation.DQN_Custom_NN.Config import ( 
    PAWS_BOT_MODEL_PATH,
    MAX_MOTOR_SPEED,
    MIN_MOTOR_SPEED,
    MAX_TURN_SPEED,
...
class Simulation:
    ...
    def __init__(self):
        ...
        self.floor_points = (-2.5, 2.5) # Min, Max

        # Possible Locations for Human
        array = np.linspace(-2.3, -1.5, num=10)
        self.waypoint_array = np.concatenate((array, np.linspace(1.5, 2.3, num=10))) 
    ...
    def step(self, old_state, action: Actions, time, old_waypoint_dist):
        ...
        if any(list(map(lambda coord: abs(coord) > self.floor_points[1], new_position))):
            ...
            return new_state, reward, done, old_waypoint_dist
    ...
    def get_state(self, bot_position, time):
        ...
        norm_vector = np.around(vector, decimals=1)
    ...
    def _move_forward(self, target_dist, reverse=False):
        ...
        while (condition):
            ...
            if self._detect_collision():
                ...
                print('COLLISION')
                break
            if np.isclose(dist_all, dist, atol=0.01).any():
